...
def replaceInlineLink(line):
    ...
    if "[link]" in linkLine:
        ...
        for i in pos:
            ...
            if link in dclTiles:
                replaceList.append((link, proto + "html/dcl-tiles/" + link, "DCL Tile: " + link))

            if link in attributes:
                #print("[replaceInlineLink] got attribut!", link)
                replaceList.append((link, proto + root_dir + link, "DCL Attribute: " + link))
...
def replaceLink(line):
    ...
    if link in dclTiles:
        path = proto + "html/dcl-tiles/" + link
        title = "DCL Tile: " + link
...
def boldFunction(line):
    ...
    if ": " in line.strip():
        ...
        both=set(dclTiles).intersection(line.split())
        #print("both:", both)
